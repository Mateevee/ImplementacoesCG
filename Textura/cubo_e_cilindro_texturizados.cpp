...
GLUquadricObj *quadratic = gluNewQuadric(); // Necessário para desenhar o cilindro

void desenharCilindro(float raio, float altura, float quads)
{
    float x, z;

    quads = quads < 3 ? 3 : quads;
    altura = altura < 0.05 ? 0.05 : altura;
    altura /= 2;
    ...
    {
        x = cos((i * (360/quads)) * PI/180.0f);
        z = sin((i * (360/quads)) * PI/180.0f);
...
void desenharCirculo(float raio, float y, float normal)
{
    float x, z;
    float aux = 0.5;
    float escala = 2;
    ...
    z = (float)raio * sin(359 * PI/180.0f);

    for (int i = 0; i < 360; i++)
    {
        glTexCoord2f(x*escala + aux, z*escala + aux);
        glVertex3f(x, y, z);
        x = (float)raio * cos(i * PI/180.0f);
        z = (float)raio * sin(i * PI/180.0f);
        glTexCoord2f(x*escala + aux, z*escala + aux);
        glVertex3f(x, y, z);
    }
...
void display(void)
{
    ...
    else // object == 4
    {
        ...
        desenharCirculo(0.25, 0.75, 1.0);
        desenharCirculo(0.25, -0.75, -1.0);
        textureManager->Bind(selected);
        desenharCilindro(0.25, 1.5, 64.0);
        glPopMatrix();
    }
    glPopMatrix();
...
int main(int argc, char** argv)
{
    ...
    cout << " * Pressione F1, F2, F3 e F4 para mudar os objetos;" << endl;
